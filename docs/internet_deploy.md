...
Chúng tôi đã khởi tạo tệp cấu hình [render.yaml](file:///home/truongan/my_agent_project/render.yaml) định nghĩa dịch vụ Web Service chạy FastAPI kèm theo ổ đĩa vật lý lưu trữ để lưu cơ sở dữ liệu `jobs.db` và cookies.
...
4. **Sao chép file cookies Facebook (`fb_cookies.json`) lên VPS:**
   * Bạn có thể sử dụng lệnh `scp` từ máy local để chuyển file cookies sang VPS một cách an toàn:
     ```bash
     scp fb_cookies.json root@ip_cua_vps:/root/FB-Job-Tracker-Matcher/
     ```
...
1. **Sử dụng Proxy dân cư (Residential Proxy):**
   Trong [fb_scraper.py](file:///home/truongan/my_agent_project/fb_scraper.py), cấu hình Proxy trong Playwright:
   ```python
